...
def manage_box_req(request):
    if request.user.is_staff:
        try:
            page_number = request.GET.get('page', '1')  # 현재 페이지 번호를 GET 요청에서 가져옴
            applys = Apply.objects.filter(progress=0).order_by('-created_at')  # progress=0 인 Apply 객체 필터링
...
def manage_pic_req(request):
    if request.user.is_staff:
        try:
            page_number = request.GET.get('page', '1')  # 현재 페이지 번호를 GET 요청에서 가져옴
            applys = Apply.objects.filter(progress=2).order_by('-created_at')  # progress=0 인 Apply 객체 필터링
            paginator = Paginator(applys, 10)  # 페이지당 10개로 페이지네이션 설정
            page_obj = paginator.get_page(page_number)  # 페이지 객체를 가져옴
...
def manage_pic_ing(request):
    if request.user.is_staff:
        try:
            page_number = request.GET.get('page', '1')  # 현재 페이지 번호를 GET 요청에서 가져옴
            applys = Apply.objects.filter(progress=3).order_by('-created_at')  # progress=0 인 Apply 객체 필터링
            paginator = Paginator(applys, 10)  # 페이지당 10개로 페이지네이션 설정
            page_obj = paginator.get_page(page_number)  # 페이지 객체를 가져옴
